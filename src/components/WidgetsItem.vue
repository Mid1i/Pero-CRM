<script setup>
	import { onFormatPrice } from "@/helpers/Formatters.js";
	...
	const compareAmounts = () => {
		...
		const difference = Number(
			(
				((props.amount - props.previousAmount) / props.previousAmount) *
				100
			).toFixed(2)
		)
	...
	<section class="content__widgets-item widget">
		...
		<div :class="['widget__row', { loading: !status }]">
			<span class="widget__amount">{{
				title === "Общая выручка" ? onFormatPrice(amount) : amount
			}}</span>
			<span v-if="status" :class="comparisonClasses">{{
				compareAmounts()
			}}</span>
		</div>
